...
from apps.tickets.choices import PROFESSION_CHOICES, SEXE_CHOICES
...
FILE_INPUT = "mt-1.5 text-sm text-ivory/70"


class BaseSignUpForm(forms.Form):
    """Champs communs aux deux types de compte (spectateur et organisateur) :
    identité de la personne (nom, prénoms, sexe, profession, photo) et
    identifiants de connexion. Voir SpectatorSignUpForm / OrganizerSignUpForm
    ci-dessous pour les champs propres à chaque rôle."""
    ...
    first_name = forms.CharField(
        label="Prénoms", max_length=150,
        widget=forms.TextInput(attrs={"class": INPUT, "placeholder": "Vos prénoms"}),
    )
    ...
    photo = forms.ImageField(
        ...
        widget=forms.ClearableFileInput(attrs={"class": FILE_INPUT}),
    )
    username = forms.CharField(
        label="Identifiant", max_length=150,
...
class SpectatorSignUpForm(BaseSignUpForm):
    """Inscription d'un spectateur : donne accès à l'historique des billets
    achetés et pré-remplit automatiquement ses informations de "visa" lors
...
class OrganizerSignUpForm(BaseSignUpForm):
    ...
    organization_logo = forms.ImageField(
        label="Logo de votre structure", required=False,
        widget=forms.ClearableFileInput(attrs={"class": FILE_INPUT}),
    )
