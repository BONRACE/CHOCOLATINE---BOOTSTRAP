...
def generate_ticket_pdf(ticket) -> bytes:
    ...
    c.drawString(6 * mm, height - 15 * mm, event.title[:30])
    ...
    # Logo de l'organisateur, en haut à droite du bandeau
    ...
    if organization and organization.logo:
        try:
            ...
            logo_size = 16 * mm
            c.drawImage(
                ImageReader(organization.logo.path),
                width - logo_size - 6 * mm, height - logo_size - 7 * mm,
                logo_size, logo_size,
                preserveAspectRatio=True, anchor="c", mask="auto",
            )
        except Exception:
            pass  # logo introuvable — on n'interrompt pas la génération du billet
    ...
    line_gap = 4.4 * mm
    details = [
        ("Sexe", ticket.get_holder_sexe_display_label() or "—"),
    ...
    # ---- Logo plateforme (attribution, bas de billet) ----
    platform_logo_path = settings.BASE_DIR / "static" / "icons" / "icon-192.png"
    footer_y = 6 * mm
    if platform_logo_path.exists():
        try:
            ...
            logo_size = 6 * mm
            logo_x = width / 2 - 18 * mm
            c.drawImage(
                ImageReader(str(platform_logo_path)),
                logo_x, footer_y - 1 * mm, logo_size, logo_size,
                preserveAspectRatio=True, mask="auto",
            )
            c.setFillColorRGB(0.5, 0.48, 0.42)
            c.setFont("Helvetica", 6.5)
            c.drawString(logo_x + logo_size + 2 * mm, footer_y, "Propulsé par EventFlow")
        except Exception:
            pass
